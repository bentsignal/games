...
- `src/components/Board.tsx`: orbitable R3F board, route picking, city labels, terrain and instanced locomotive geometry.
- `assets/locomotive.blend`: original editable Blender source. `scripts/create_locomotive.py` recreates it inside Blender. The browser uses `public/models/locomotive.glb`.
...
See [docs/SOURCES.md](docs/SOURCES.md) for factual data verification and asset provenance. This is an unofficial independent implementation, with original interface and 3D artwork. Ticket to Ride is a trademark of Days of Wonder; the original game was designed by Alan R. Moon. No official board or card artwork is bundled.
