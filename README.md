...
Use Node 24+. Setup uses the existing `BSX:games` project. Setup creates an isolated Convex development deployment for this checkout, expiring in seven days, and configures local auth and the Grams Worker. No Google OAuth client or Cloudflare login is needed.
